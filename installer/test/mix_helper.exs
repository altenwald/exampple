...
defmodule MixHelper do
  ...
  def in_tmp_project(which, function) do
    conf_before = Application.get_env(:exampple, :generators) || []
    path = Path.join([tmp_path(), random_string(10), to_string(which)])
    ...
    after
      File.rm_rf!(path)
      Application.put_env(:exampple, :generators, conf_before)
    end
  end
  ...
  def with_generator_env(new_env, fun) do
    Application.put_env(:exampple, :generators, new_env)
    try do
      fun.()
    after
      Application.delete_env(:exampple, :generators)
    end
  end
